...
namespace MeshEditor.LayerManager.Compression
{
	...
	/// </remarks>
	internal sealed class SVD
	{
		private SVD(Vector<double> s, Matrix<double> u, Matrix<double> vt, bool vectorsComputed)
		{
			S = s;
			U = u;
			VT = vt;
		...
		private readonly bool vectorsComputed;

		/// <summary>
		/// Gets the singular values (Σ) of matrix in ascending value.
		/// </summary>
		public Vector<double> S { get; }
		...
		public static SVD Create(Matrix<double> matrix, bool computeVectors = true)
		{
			...
			return new SVD(s, u, vt, computeVectors);
		}
