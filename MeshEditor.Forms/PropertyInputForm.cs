...
namespace MeshEditor.WinUI
{
	public partial class PropertyInputForm : Form
	{
		...
		public PropertyInputForm(string caption, string infoText, SceneFacade sceneFacade)
			: this(caption, infoText, SavedValue)
		{
			this.sceneFacade = sceneFacade;
		}

		public PropertyInputForm(string caption, string infoText, string boxValue)
		{
			InitializeComponent();
		...
		private void linkLabelEditPropertyComment_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
		{
			if (validateInput())
			{
				int value;
				if (int.TryParse(InputValue, out value))
				{
					...
					string oldComment;
					statistics.PropertyComments.TryGetValue(property, out oldComment);

					InputValueForm inputValueForm = new InputValueForm("Set property comment", "Property " + property + ":", oldComment);
					inputValueForm.Width = 500;
					if (inputValueForm.ShowDialog() == System.Windows.Forms.DialogResult.OK)
					{
						// save comment
						...
						if (oldComment != inputValueForm.InputValue)
						{
